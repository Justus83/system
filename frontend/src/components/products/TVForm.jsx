...
const TVForm = ({ storeId, onSuccess, onError }) => {
  ...
  const fetchEnums = async () => {
    try {
      ...
      const [brandsRes, modelsRes, colorRes, screenSizeRes, resolutionRes, sourceRes] = await Promise.all([
        ...
        fetch(`http://localhost:5001/api/enums/screen-sizes?storeId=${storeId}`, { headers }),
        fetch(`http://localhost:5001/api/enums/resolutions?storeId=${storeId}`, { headers }),
      ...
      setEnumOptions({ brands, models, colors, screenSizes, resolutions, sources });
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await productsAPI.createTV(payload);
      ...
      console.error('Failed to add TV:', err);
      onError?.(err.message || 'Failed to add TV');
    } finally {
      setSaving(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <SelectWithAdd
          ...
          options={enumOptions.screenSizes}
        ...
        <SelectWithAdd
          ...
          options={enumOptions.resolutions}
